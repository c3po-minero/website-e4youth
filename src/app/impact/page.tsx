...
export default function ImpactPage() {
  return (
    <>
      <Hero title="Impact Beyond Numbers: Stories of Real Change" lead="E4 Youth measures impact across learning, visibility, and opportunity, capturing growth over time through portfolios, applied work, and progression into next-step pathways." bgImage="https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=1920&h=1080&fit=crop" icon="📊" buttons={[{ label: 'View Impact Highlights', href: '#highlights', variant: 'primary' }, { label: 'Partner on Evaluation', href: '#evaluation', variant: 'secondary' }]} />

      <Section icon="🧭" title="Beyond Traditional Metrics" lead="E4 Youth designs evaluation that honors participant voice, documents community-defined success, and tracks authentic progression.">
        <div className="grid grid-2">
          {[
            { icon: '✅', title: 'Participant-Centered', desc: 'Success metrics developed with participants' },
            { icon: '👥', title: 'Community-Defined', desc: 'Impact indicators reflect community values' },
            { icon: '📈', title: 'Longitudinal Tracking', desc: 'We measure growth over time' },
            { icon: '📁', title: 'Portfolio-Based', desc: 'Participants document their own learning' },
      ...
      <Section icon="🧊" title="Learning, Visibility, and Opportunity">
      ...
      <Section icon="📋" title="How We Gather and Honor Participant Voice" id="evaluation">
        <div className="grid grid-2">
          {[
            { icon: '📁', title: 'Portfolio-Based Assessment', desc: 'Participants document their learning through creative portfolios with work samples, reflections, and peer feedback.' },
            { icon: '💬', title: 'Community Feedback Integration', desc: 'Family members, community partners, and mentor networks provide regular feedback on participant growth.' },
            { icon: '🎤', title: 'Longitudinal Participant Interviews', desc: 'Annual interviews use storytelling methodologies to document learning experiences and community connections.' },
            { icon: '🤝', title: 'Partner Organization Evaluation', desc: 'Schools, employers, and community organizations provide feedback on readiness levels and ongoing contribution.' },
          ].map(c => <div className="card" key={c.title}><span className="card-icon">{c.icon}</span><h3 className="card-title">{c.title}</h3><p className="card-description">{c.desc}</p></div>)}
        </div>
      </Section>

      <Section icon="⭐" title="2024 Impact Snapshot" id="highlights">
        <div className="grid grid-3">
          <div className="card">
      ...
      <Section icon="❤️" title="Impact in Their Own Words">
        <div className="grid grid-3">
          {[
      ...
      <Section icon="🚀" title="Ready to See Impact in Action?">
