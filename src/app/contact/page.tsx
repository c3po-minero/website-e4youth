...
export default function ContactPage() {
  ...
  return (
    <>
      ...
      <Section icon="🧭" title="Multiple Ways to Engage" id="get-involved">
        <div className="grid grid-2">
          {[
            { icon: '🚀', title: '"I\'m a young person (ages 12-25)"', desc: 'Discover programs that combine storytelling with digital literacy, career preparation, and community connection.', href: '/programs', cta: 'Explore Youth Programs' },
            { icon: '🏠', title: '"I\'m a parent or family member"', desc: 'Join family engagement opportunities that honor community knowledge and support intergenerational learning.', href: '#family-engagement', cta: 'Join Family Programming' },
            { icon: '🤝', title: '"I\'m a professional who wants to mentor"', desc: 'Become a community mentor, share industry knowledge, and connect with motivated young people.', href: '/partner', cta: 'Become a Community Mentor' },
            { icon: '🔗', title: '"I represent an organization"', desc: 'Explore collaboration opportunities with schools, businesses, and foundations.', href: '/partner', cta: 'Explore Partnership Options' },
          ].map(c => (
      ...
      <Section icon="📧" title="Get In Touch" lead="Have questions? Want to explore partnership opportunities? Send us a message." id="contact-form">
        <div className="grid grid-2">
          <div>
            {submitted ? (
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true) }}>
                ...
                <div className="form-group"><button type="submit" className="btn btn-primary btn-full">📩 Send Message</button></div>
          ...
          <div>
            ...
            <div className="card mt-lg">
              ...
              <p style={{ fontSize: '0.9rem' }}>We offer: Spanish language support, extended response times, phone and video call options, in-person meetings, and alternative communication formats.</p>
            </div>
          </div>
        </div>
      </Section>

      <Section icon="📋" title="Direct Contact Information">
        <div className="grid grid-3">
          {[
            { icon: '📧', title: 'General Information', desc: 'Email: hello@e4youth.org\nPhone: (512) 555-0123' },
            { icon: '🎓', title: 'Youth Programs', desc: 'Email: programs@e4youth.org\nAlex Martinez, Youth Engagement Coordinator' },
            { icon: '🏠', title: 'Family Engagement', desc: 'Email: families@e4youth.org\nJennifer Park, Family Coordinator' },
            { icon: '🤝', title: 'Professional Mentorship', desc: 'Email: mentors@e4youth.org\nSarah Chen, Professional Development Coordinator' },
            { icon: '💼', title: 'Workforce Partnerships', desc: 'Email: careers@e4youth.org\nDavid Kim, Workforce Engagement Manager' },
            { icon: '🏫', title: 'Educational Partnerships', desc: 'Email: schools@e4youth.org\nDr. Maria Rodriguez, Educational Collaboration Director' },
          ].map(c => (
            <div className="card" key={c.title}>
              <span className="card-icon">{c.icon}</span>
              <h3 className="card-title">{c.title}</h3>
      ...
      <Section icon="📅" title="Join Us: Regular Community Events" id="family-engagement">
        <div className="grid grid-3">
          <div className="card"><span className="card-icon">👥</span><h3 className="card-title">E4 Live Networking Events</h3><p className="card-description"><strong>When:</strong> First Thursday of each month, 6:00-8:00 PM<br /><strong>Cost:</strong> Free, no registration required</p></div>
          <div className="card"><span className="card-icon">⭐</span><h3 className="card-title">Quarterly Community Showcases</h3><p className="card-description"><strong>When:</strong> End of each program session<br /><strong>Duration:</strong> 2-3 hours with flexible arrival</p></div>
          <div className="card"><span className="card-icon">❤️</span><h3 className="card-title">Annual Community Celebration</h3><p className="card-description"><strong>When:</strong> Early summer (date announced annually)<br /><strong>Special Features:</strong> Intergenerational community connection</p></div>
        </div>
      </Section>

      <Section icon="➡️" title="Ready to Connect?" lead="Every meaningful relationship starts with conversation. We're here to help you find the perfect way to engage.">
